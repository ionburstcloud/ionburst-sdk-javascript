...
class Settings {
    ...
    async manage_config_file() {
        ...
        if (fs.existsSync(config_file)) {
            ...
            // Merge to config.json  
            if (_this.ionburst_uri && !obj.Ionburst.IonBurstUri) {
                obj.Ionburst.IonBurstUri = _this.ionburst_uri;
            }
            let json = JSON.stringify(obj); //convert it back to json
            fs.writeFile(config_file, json, 'utf8', function (err, data){
                if (err){
                    throw (err);
                }
            });
            ...
            if (this.ionburst_uri) {
                obj.Ionburst.IonBurstUri = this.ionburst_uri;
            }
            let json = JSON.stringify(obj); //convert it back to json
            fs.writeFile(config_file, json, 'utf8', function (err, data){
                if (err){
    ...
    async manage_credentials() {
        ...
        if (!credentials_file) {
            credentials_file = home_directory + '/.ionburst/credentials.ini';
        }
        if (this.TraceCredentialsFile){
