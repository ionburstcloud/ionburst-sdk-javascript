...
class APIHandler {
  ...
  async headData(id, deferred = false, depth = 0) {
    if (!id) {
      return { status: 400, statusText: "id must be specified to query object" }
    }
    let url = (deferred ? `${this.ionburst_uri}api/data/deferred/start/${id}` : `${this.ionburst_uri}api/data/${id}`);
    try {
      ...
      if (error.response.status === 401 && !depth) {
        ...
        return await this.headData(id, deferred, 1);
      }
      return error.response;
    }
  }

  async headSecrets(id, deferred = false, depth = 0) {
    if (!id) {
      return { status: 400, statusText: "id must be specified to query secret" }
    }
    let url = (deferred ? `${this.ionburst_uri}api/data/deferred/start/${id}` : `${this.ionburst_uri}api/secrets/${id}`);
    try {
      let res = await axios.head(url, {
        responseType: 'arraybuffer',
        headers: {
      ...
      if (error.response.status === 401 && !depth) {
        ...
        return await this.headSecrets(id, deferred, 1);
      }
      return error.response;
    }
  }
