...
class Client {
  ...
  async _checkinit_for_classification() {
    if (!this.settings.ionburst_uri) {
      await this.settings.init_uri();
      this._apiHandler.ionburst_uri = this.settings.ionburst_uri;
    }
  }
  ...
  async getClassifications(callback) {
    await this._checkinit_for_classification();
    if (typeof callback !== 'function') {
